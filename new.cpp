...
extern double eob_cost[63], eob_cost_2[63];       // cost of going to EOB from current state
extern double dist[64][16],dist_2[64][16];        // store pre-calculated distortion
extern double d[64][11], d_2[64][11];             // mean square distortion of the 64 coef. quantized to level s
extern double ending_cost[63], ending_cost_2[63]; // cost of ending the block after the current state 
extern double cost, cost_2;                       // temporal cost to current state for one path 
extern double dist_inc, dist_inc_2;				  // distorting increment from incoming state to current state
extern double curr_min, curr_min_2;               // current minimum cost to present state 
extern double tempd;
extern double ent_dc[12];         // entropy of DC size group
extern double ent_ac[256];        // entropy of each (r,s) pair
extern double maxnumber;
...
extern int final_nzc;
extern int s_min[11], s_max[11];
extern int c_abs[64],c_abs_2[64];              // absolute values of the DCT coefs. in one block 
extern int qtbl_1d[64], qtbl_1d_thrld[64], Q[64]; 
extern int id[64][11], id_2[64][11];              // the index to send when the coef. is quantized to level s
extern int indsi[64], indsi_2[64];                // stort the size of each index

extern double enc_add_cost[64][16][11],  enc_add_cost_2[64][16][11];
extern double dist_inc_cost[64][16][11], dist_inc_cost_2[64][16][11];
int zz[64];
...
void update_cost_new(int i,int j,int size, double dist_inc_cost, double enc_add_cost, int ind, statenode* state)
{
	   double  cost_f;
	   cost_f=state[i-j-1].min_cost+enc_add_cost;
	   if (cost_f<curr_min)
	   {
	   	  record_inf(state, i,j,size, ind, dist_inc_cost, cost_f);
		  curr_min=cost_f;
	   }
}

void update_cost_new_2(int i,int j,int size, double dist_inc_cost, double enc_add_cost, int ind, statenode* state)
{
	   double  cost_f;
	   cost_f=state[i-j-1].min_cost+enc_add_cost;
	   if (cost_f<curr_min_2)
	   {
	   	  record_inf(state, i,j,size, ind, dist_inc_cost, cost_f);
		  curr_min_2=cost_f;
	   }
...
void search_min_cost_new(statenode* state_a0, statenode* state_b0, int i, int j)
{
	int size;
	switch (indsi[i])
    {
     case 0:
	  for (size=1; size<3; size++)
	    update_cost_new(i,j,size,dist_inc_cost[i][j][size],enc_add_cost[i][j][size], id[i][size],state_a0);
     break;
     case 1:
	 ...
	 switch (indsi_2[i])
	 {
	  case 0:
	   for (size=1; size<3; size++)
		 update_cost_new_2(i,j,size,dist_inc_cost_2[i][j][size],enc_add_cost_2[i][j][size],id_2[i][size], state_b0);
      break;
      case 1:
	   for (size=1; size<4; size++)
		 update_cost_new_2(i,j,size,dist_inc_cost_2[i][j][size],enc_add_cost_2[i][j][size],id_2[i][size], state_b0);
...
void update_cost_2(int i,int j,int size, double dist, double d, int ind, statenode* state )
{
	   double dist_inc_f, cost_f;
	   dist_inc_f=dist+d;
	   cost_f=state[i-j-1].min_cost+dist_inc_f+ent_ac[(j<<4)+size];
...
void init_jwc_block(int *dctcoef_1b, int *dctcoef_2b)
{
 int i,j,k,ind1,ind2,tmp,s,size;
  //initialize the first column of dist[][] and dist_2[][](SAME for all block, go outside later)
  for (i=2; i<64; i++)
  {
	dist[i][0]=0; 
    dist_2[i][0]=0; 
  ...
  //for each 2_8x8 block   //use the absolute value instead of MSE as the distortion criterion
  for (i=0; i<64; i++)
  {
	c_abs[i]=(dctcoef_1b[i]<0)?(-dctcoef_1b[i]):dctcoef_1b[i];
	c_abs_2[i]=(dctcoef_2b[i]<0)?(-dctcoef_2b[i]):dctcoef_2b[i];
  }

  //calculate the costs of going to EOB from the ith state
  eob_cost[62]=sqr(c_abs[63])+ent_ac[0];
  eob_cost_2[62]=sqr(c_abs_2[63])+ent_ac[0];

  for (i=62; i>0; i--)  // state 63 does not follow EOB
  {
	eob_cost[i-1]=eob_cost[i]+sqr(c_abs[i]);
	eob_cost_2[i-1]=eob_cost_2[i]+sqr(c_abs_2[i]);
  }
  ...
  for (i=2; i<16; i++)     // for i=2~15
	for (j=1; j<i; j++)   // for j=1~(i-1)
	{
	    ...
	    dist_2[i][j]=dist_2[i][j-1]+sqr(c_abs_2[i-j]);
	}
  ...
  for (i=1; i<64; i++)
  {
     ind1=quantize1c(dctcoef_1b[i],i);
	 tmp=ind1;
	 if(tmp<0) tmp=-tmp;
     s=0;
     while(tmp)
     ...
     indsi[i]=s;    d[i][s]=sqr(dctcoef_1b[i]-ind1*qtbl_1d[i]);  id[i][s]=ind1;
	 switch (s)
	 {
	    case 0:
		  for (size=1; size<3; size++)
     ...
     ind2=quantize1c(dctcoef_2b[i],i);
	 tmp=ind2;
	 if(tmp<0) tmp=-tmp;
     ...
     indsi_2[i]=s;  d_2[i][s]=sqr(dctcoef_2b[i]-ind2*qtbl_1d[i]);  id_2[i][s]=ind2;
	 ...
	 switch (s)
	 {
	    case 0:
		  for (size=1; size<3; size++)
		    {
		     d_2[i][size]=sqr(c_abs_2[i]-s_min[size]*qtbl_1d[i]);
		     id_2[i][size]=(dctcoef_2b[i]<0)?(-s_min[size]):s_min[size];
	        }
          break;
	    ...
	    case 10:
		  d_2[i][9]=sqr(c_abs_2[i]-s_max[9]*qtbl_1d[i]);
		  id_2[i][9]=(dctcoef_2b[i]<0)?(-s_max[9]):s_max[9];
          break;

        default: //2~9 
          d_2[i][s-1]=sqr(c_abs_2[i]-s_max[s-1]*qtbl_1d[i]);
		  id_2[i][s-1]=(dctcoef_2b[i]<0)?(-s_max[s-1]):s_max[s-1];
          d_2[i][s+1]=sqr(c_abs_2[i]-s_min[s+1]*qtbl_1d[i]);
		  id_2[i][s+1]=(dctcoef_2b[i]<0)?(-s_min[s+1]):s_min[s+1];
		  break;
	 } //end of switch
  }
  ...
  {	 
	 ...
	 for (j=0; j<=k; j++)
	   {
	     switch (indsi[i])
		 {
	      case 0:
		    for (size=1; size<3; size++)
		     {
			  dist_inc_cost[i][j][size]=dist[i][j]+d[i][size];
			  enc_add_cost[i][j][size]=dist_inc_cost[i][j][size]+ent_ac[(j<<4)+size];
	  ...
	  switch (indsi_2[i])
		 {
	      case 0:
		    for (size=1; size<3; size++)
		     {
			  dist_inc_cost_2[i][j][size]=dist_2[i][j]+d_2[i][size];
			  enc_add_cost_2[i][j][size]=dist_inc_cost_2[i][j][size]+ent_ac[(j<<4)+size];
	 ...
	 if (i>15&&i<63)
	    {	
		 dist_inc_cost[i][j][0]=dist[i][15]+sqr(c_abs[i]);
		 enc_add_cost[i][j][0]=dist_inc_cost[i][j][0]+ent_ac[0xF0];

		 dist_inc_cost_2[i][j][0]=dist_2[i][15]+sqr(c_abs_2[i]);
		 enc_add_cost_2[i][j][0]=dist_inc_cost_2[i][j][0]+ent_ac[0xF0];
...
void search_min_cost(statenode* state_a0, statenode* state_b0, int i, int j)
{
	...
	switch (indsi[i])
    {
	 ...
	 switch (indsi_2[i])
	 {
	  case 0:
	   for (size=1; size<3; size++)
		 update_cost_2(i,j,size,dist_2[i][j],d_2[i][size],id_2[i][size], state_b0);
      break;
      case 1:
	   for (size=1; size<4; size++)
		 update_cost_2(i,j,size,dist_2[i][j],d_2[i][size],id_2[i][size], state_b0);
...
void opti_block(int row_b, int col_b, int *dctcoef_1b, statenode *state, int *stack, int *pointer,
				int *counts, int *total, double *distortion_t, double *rate_total, double *cost_t)
{
 ...
 statenode final_state;
  ...
  for (i=1; i<64; i++)
	{
     ...
     indsi[i]=s;
     d[i][s]=sqr(dctcoef_1b[i]-ind*qtbl_1d[i]);
	 id[i][s]=ind;
	 switch (s)
	   {
	    case 0:
  ...
  for (i=1; i<64; i++)
	...
	else         // the 63th AC coef. is the non-zero
	   {
		stack[sp]=63;
